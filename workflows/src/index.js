...
app.get('/api/members', async (req, res) => {
  try {
    const { results } = await env.db.prepare('SELECT * FROM chats ORDER BY joined_date DESC').all();

    res.json({ success: true, members: results });
  } catch (error) {
    res.status(400).json({ success: false, error: 'Failed to fetch members' });
  }
});
...
app.get('/api/members/:id', async (req, res) => {
  try {
    ...
    const { results } = await env.db.prepare('SELECT * FROM chats WHERE id = ?').bind(id).all();
...
app.post("/api/members", async (req, res) => {
  try {
    ...
    const result = await env.db.prepare(
      "INSERT INTO chats (name, email, joined_date) VALUES (?, ?, ?)"
    )
...
app.delete("/api/members/:id", async (req, res) => {
  try {
    ...
    const result = await env.db.prepare("DELETE FROM chats WHERE id = ?")
      .bind(id)
      .run();
